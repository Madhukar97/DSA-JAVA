...
//https://www.codingninjas.com/studio/problems/ninja-s-game_1264945?leftPanelTab=0
//Striver DP series vid 44
public class IncreasingTripletSubsequence {
    public static int ninjaGame(ArrayList<Integer> arr, int n) {
        int[] nums = new int[n];
        ...
        return recMem(n-1,n,0,nums,dp) ? 1 : 0;
    }
    //Recursion with Memoization(Top-Down approach) solution TC = O(n*2) , SC = O(n) + dp[n][n+1]
    public static boolean recMem(int i, int rightIndex, int count, int[] nums, int[][] dp){
        if(i < 0){
            return count >= 3;
        }
    ...
    public static int ninjaGameTabulation(ArrayList<Integer> nums, int n) {
